        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    import pickle
                    cpu_tensor = result.detach().cpu()
                    if cpu_tensor.size(1) > 1:
                        with open('original.pkl', 'ab') as f:  # append binary
                            pickle.dump(cpu_tensor, f)
                    ...
                    cpu_tensor = result.detach().cpu()
                    if cpu_tensor.size(1) > 1:
                        with open('lora.pkl', 'ab') as f:  # append binary
                            pickle.dump(cpu_tensor, f)
