        ...
        def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
            if self.disable_adapters:
                ...
                for active_adapter in self.active_adapters:
                    ...
                    # if lora_type != "":
                    #     if lora_type == "query":
                    #         x = F.linear(x, share_weights_query.to(device))
                    #         # lora_A.weight = share_weights_key.to(device)
                    #     elif lora_type == "key":
                    #         x = F.linear(x, share_weights_key.to(device))
                    #         # lora_A.weight = share_weights_key.to(device)
                    #     elif lora_type == "value":
                    #         x = F.linear(x, share_weights_value.to(device))
                    #         # lora_A.weight = share_weights_key.to(device)
                    # else:
