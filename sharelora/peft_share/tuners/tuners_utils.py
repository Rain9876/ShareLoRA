...
class BaseTuner(nn.Module, ABC):
    ...
    def __init__(self, model, peft_config: Union[PeftConfig, dict[str, PeftConfig]], adapter_name: str) -> None:
        ...
        r = 64
        self.share_weights = {"q_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "k_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "v_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "o_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "gate_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "up_proj": torch.nn.Parameter(torch.randn(r, 4096)),
                              "down_proj": torch.nn.Parameter(torch.randn(r, 14336))}

        for w in self.share_weights.values():            
            torch.nn.init.normal_(w, std=1 / r)
