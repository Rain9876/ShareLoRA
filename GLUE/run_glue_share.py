...
def main():
    ...
    ################################## ##################################
    ################## Share LoRA Modules
    # LoRA Paper adapts Query and Value for GLUE Benchmark
    ################################## ##################################
            ...
            if  "query" in name and "bias" not in name:
                ...
                if shareA:
                    if shared_lora_A_query is None:
                        shared_lora_A_query = module.lora_A
                    else:
                        print(f"value assign {name}")
                        setattr(module, 'lora_A', shared_lora_A_query)
                        module._parameters['lora_A'] = shared_lora_A_query  # Re-register the shared parameter
                
                if shareB:
                    if shared_lora_B_query is None:
                        shared_lora_B_query = module.lora_B
                    else:
                        print(f"value assign {name}")
                        setattr(module, 'lora_B', shared_lora_B_query)
                        module._parameters['lora_B'] = shared_lora_B_query  # Re-register the shared parameter

            if  "value" in name and "bias" not in name:
                print(f"{'value' in name}")
                if shareA:
                    if shared_lora_A_value is None:
                        shared_lora_A_value = module.lora_A
                    else:
                        print(f"value assign {name}")
                        setattr(module, 'lora_A', shared_lora_A_value)
                        module._parameters['lora_A'] = shared_lora_A_value  # Re-register the shared parameter

                if shareB:
                    if shared_lora_B_value is None:
                        shared_lora_B_value = module.lora_B
                    else:
                        print(f"value assign {name}")
                        setattr(module, 'lora_B', shared_lora_B_value)
                        module._parameters['lora_B'] = shared_lora_B_value  # Re-register the shared parameter

    def set_trainable_parameters(model, require_grad=True):
        """
        ...
        for name, param in model.named_parameters():
            ...
            # Whether frozen classifier or not
    ...
    print(model) 
    set_trainable_parameters(model, True)

    seen_parameters = set()
    unique_parameters = 0
    for param in model.parameters():
        # Check if any parameter's memory location is already seen
        if id(param) not in seen_parameters and param.requires_grad:
            seen_parameters.add(id(param))
            unique_parameters += param.numel()
    print("Number of unique trainable parameters:", unique_parameters)
    ...
    trainer = Trainer(
        model=model,
        args=training_args,
        #train_dataset=train_dataset.shuffle(seed=training_args.seed).select(range(5)) if training_args.do_train else None,
    ...
    if training_args.do_train:
        ...
        if last_checkpoint is not None:
            ...
            if AutoConfig.from_pretrained(model_args.model_name_or_path).num_labels == num_labels:
                checkpoint = model_args.model_name_or_pathh
        print(checkpoint)
