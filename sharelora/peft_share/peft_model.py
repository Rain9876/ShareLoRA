    ...
    **Attributes**:
    ...
    def get_prompt(self, batch_size: int, task_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if isinstance(self.base_model, PeftModel) and isinstance(self.base_model.base_model, LoraModel) and False:
                # cls = bnb.nn.Linear4bit if args.bits == 4 else (bnb.nn.Linear8bitLt if args.bits == 8 else
                # torch.nn.Linear)
                from .tuners.lora.bnb import Linear8bitLt, Linear4bit
                lora_attn_module_names = {}
                for name, module in self.base_model.named_modules():
                    if isinstance(module, Linear4bit):
                        if "attn" in name and ("k_" in name or "v_" in name):
                            lora_attn_module_names[name] = (module)

                # Convert lora_attn_module_names to a list of modules
                lora_attn_modules = [module for name, module in sorted(lora_attn_module_names.items())]

                # past_key_values = past_key_values.view(batch_size, peft_config.num_virtual_tokens, 2,
                #                                        peft_config.num_layers, peft_config.token_dim)

                past_key_values = past_key_values.view(batch_size, peft_config.num_virtual_tokens, 2,
                                                       peft_config.num_layers, 64)

                # Initialize lists for key_states and value_states
                key_states = []
                value_states = []

                # Loop through lora_attn_modules and calculate key_states and value_states
                for i, m in enumerate(lora_attn_modules):
                    module_input = past_key_values[:, :, i % 2, i // 2, :]
                    if i % 2 == 0:
                        key_states.append(m(module_input, past_key_value=True, lora_alpha=64))
                    else:
                        value_states.append(m(module_input, past_key_value=True, lora_alpha=64))
                # print("Do key Value Lora")
                # Stack key_states and value_states along the specified dimensions
                key_states = torch.stack(key_states)
                value_states = torch.stack(value_states)

                # Concatenate key_states and value_states
                data = torch.cat([key_states, value_states], dim=0).permute(1, 2, 0, 3)
                past_key_values = data.contiguous().view(batch_size, peft_config.num_virtual_tokens,
                                                         peft_config.num_layers * 2 * peft_config.token_dim)
            # print("here")
            ...
            # print(past_key_values[0][1])

            from model.cache_utils import UnfreezingCache, Cache
            if not isinstance(past_key_values, Cache) and self.unfreeze_layers > 0:
                past_key_values = UnfreezingCache.from_legacy_cache(past_key_values, True, self.unfreeze_layers)
...
class PeftModelForSequenceClassification(PeftModel):
    ...
    **Attributes**:
    ...
    def _prefix_tuning_forward(
        ...
        if "past_key_values" in fwd_params or isinstance(self.base_model, PeftModel):
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        if not peft_config.is_prompt_learning:
            ...
            # print(f"Run LORA Tuning Once {self.active_peft_config.is_prompt_learning}")
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            past_key_values = self.get_prompt(batch_size)
            # print(f"Run Prefix Tuning Once {self.active_peft_config.is_prompt_learning}")
            return self.base_model(
    ...
    def generate(self, **kwargs):
        ...
        print(f'Start Generate {"Prefix" if self.active_peft_config.is_prompt_learning else "Lora"}')

        if hasattr(self.base_model, "model"):
            self.base_model.model.generation_config = self.generation_config
            print(f'here for model {self.active_peft_config.is_prompt_learning}')
        else:
            self.base_model.generation_config = self.generation_config
            print(f'here for no model  {self.active_peft_config.is_prompt_learning}')
        ...
        try:
            ...
            self.base_model.prepare_inputs_for_generation = self.base_model_prepare_inputs_for_generation
            print(f"Run here for prompt {self.active_peft_config.is_prompt_learning}")
            return outputs
